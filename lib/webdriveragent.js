...
const AGENT_TESTWA_LOG_PREFIX = 'TestWaAgent';
...
class WebDriverAgent {
  ...
  constructor (xcodeVersion, args = {}) {
    ...
    this.realDeviceLogger = _path2['default'].resolve(__dirname, '..', 'deviceconsole', 'deviceconsole');
    ...
    function isPertinentLogLine (line) {
      return line.length &&
            (line.indexOf(AGENT_LOG_PREFIX) !== -1 ||
             ...
             line.indexOf(AGENT_TESTWA_LOG_PREFIX) !== -1 ||
             line.indexOf(SIM_BRIDGE_LOG_PREFIX) !== -1);
    }

    if (this.realDevice && this.realDeviceLogger.indexOf('deviceconsole') !== -1) {
      ...
      logs.on('output', (stdout, stderr) => {
        ...
        if (stdout.indexOf(errorString) !== -1 || stderr.indexOf(errorString) !== -1) {
          ...
          let msg = `The real device logger '${this.realDeviceLogger}' was ` +
                    ...
                    `'ideviceinstaller' ('npm install -g ideviceinstaller') and ` +
